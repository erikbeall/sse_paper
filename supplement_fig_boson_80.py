...
c=0
# 2a is maxima of full set of 80C boson data vs radius
axes[c].plot(orig_radii80, orig_cmaximas80, 'k.')
axes[c].legend([r"$T_{T}$ = 80$^\circ$C"], fontsize=14)
...
c=1
tgt80 = np.polyfit(1/np.power(orig_radii80[orig_radii80>2.75], 0.5), orig_cmaximas80[orig_radii80>2.75], 1)[1]
axes[c].plot(orig_radii80, tgt80 - orig_cmaximas80, 'k.')
axes[c].legend([r"$T_{T}$ = 80$^\circ$C"], fontsize=14)
axes[c].set_xlim([-1.5, 90])
axes[c].grid(True)
...
c=2
axes[c].plot(orig_radii80, pcts80, 'k.')
axes[c].legend([r"$T_{T}$ = 80$^\circ$C"], fontsize=14)
...
plt.savefig('sse_figure_3_supplement80.png')
